...
const Achievements = () => {
  ...
  return (
    <section id="achievements" className="text-white bg-primary py-8">
      <h1 className="section-title text-center text-6xl font-bold mb-10">Achievements</h1>
      <ul
        ref={containerRef}
        className="w-full max-w-6xl mx-auto py-20 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8"
      >
        {achievements.map(({ title, image, color, descr }, index) => (
          ...
          >
            ...
            <div className="title text-lg font-semibold uppercase mt-2">{title}</div>
            <div className="descr text-sm text-gray-200 whitespace-pre-line">{descr}</div>
          </li>
        ))}
      </ul>
