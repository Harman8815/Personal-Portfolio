...
const MoreProjects = () => {
  ...
  const uniqueTags = [
    "All",
    ...Array.from(
      new Set(myProjects.flatMap((project) => project.tags.map((tag) => tag.name)))
    ),
  ];
  ...
  useEffect(() => {
    ...
    function scrollInAnimation(card) {
      gsap.fromTo(card, { y: 100, opacity: 0, scale: 0.9 }, { y: 0, opacity: 1, scale: 1, duration: 1.2, ease: "power3.out" });
    }
    ...
    function scrollReEnterAnimation(card) {
      gsap.fromTo(card, { y: -100, opacity: 0, scale: 0.9 }, { y: 0, opacity: 1, scale: 1, duration: 1, ease: "power3.out" });
    }
  ...
  return (
    ...
    >
      <h2 className="text-6xl font-bold laptop:text-5xl desktop:text-6xl mb-6">All Projects</h2>

      {/* Tabs and Search */}
      <div className="flex flex-col laptop:flex-row justify-between items-center w-full gap-4 mb-8 px-18">
        {/* Tabs */}
        <div className="flex flex-wrap gap-3 w-[72%]">
          {uniqueTags.map((tag) => (
            <button
              key={tag}
              onClick={() => { setActiveTab(tag); setVisibleCount(9); }}
              className={`px-4 py-2 rounded-full ${
                activeTab === tag ? "bg-white text-gray-900" : "bg-gray-700 hover:bg-gray-600"
        ...
        <div>
          <input
            ...
            className="px-4 py-2 rounded-full bg-gray-700 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-white"
            value={searchQuery}
